    ...
    return (
        <div className="event-card" onClick={() => onClick(event)}>
            ...
            <div className="event-poster">
                ...
                <span className="event-type-badge">{event.type === 'public' ? '🌍 Public' : '🔒 Private'}</span>
                {isRegistered && <span className="event-registered-badge">✓ Registered</span>}
            </div>
            <div className="event-body">
                <h3 className="event-title">{event.title}</h3>
                <div className="event-info">
                    <div className="info-item"><span className="info-icon">📅</span><span>{new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span></div>
                ...
                <div className="event-capacity">
                    <div className="capacity-bar">
                        <div className="capacity-fill" style={{ width: pct + '%', background: pct >= 90 ? '#e94560' : pct >= 70 ? '#f39c12' : 'linear-gradient(90deg,#667eea,#764ba2)' }} />
                    </div>
                    <span className="capacity-label">👥 {event.participants}/{event.maxParticipants}</span>
                </div>
            ...
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-banner">
                    ...
                    <div className="modal-banner-overlay">
                        <span className="event-type" style={{ background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(8px)' }}>{event.type === 'public' ? '🌍 Public Event' : '🔒 Private Event'}</span>
                        <h3 className="modal-banner-title">{event.title}</h3>
                    </div>
...
function CreateEventModal({ onClose, onCreate }) {
    const [formData, setFormData]             = useState({ title: '', description: '', date: '', time: '', location: '', type: 'public', maxParticipants: 100, image: 'images/event1.jpg' });
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="create-event-grid">
                            ...
                            <div className="form-group form-group-full">
                                ...
                                {formData.image && <img src={formData.image} alt="preview" style={{ marginTop:'0.75rem', width:'100%', height:160, objectFit:'cover', borderRadius:12, border:'2px solid #f0f0f0' }} onError={e => e.target.style.display='none'} />}
                            </div>
